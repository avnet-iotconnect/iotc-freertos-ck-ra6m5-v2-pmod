 ...
 * , not for production. No performance or suitability for any purpose including medical devices is guaranteed or
...
volatile float32_t      gs_demo_air_quality;
rm_zmod4xxx_raw_data_t  oaq_raw_data;

/**********************************************************************************************************************
 * Function Name: start_oaq_1st_gen_demo
 * Description  : .
 * Return Value : .
 *********************************************************************************************************************/
void start_oaq_1st_gen_demo(void)
{
    ...
    switch (sequence)
    {
...
#endif
            /* Start measurement */
            err = g_zmod4xxx_sensor2.p_api->measurementStart(g_zmod4xxx_sensor2.p_ctrl);
            if (FSP_SUCCESS == err)
            {
        ...
        {
            /* Clear status */
            g_i2c_callback_status2 = DEMO_CALLBACK_STATUS_WAIT;

            /* Get status */
            err = g_zmod4xxx_sensor2.p_api->statusCheck(g_zmod4xxx_sensor1.p_ctrl);
            if (FSP_SUCCESS == err)
            {
        ...
        case DEMO_SEQUENCE_5 :
        {
            ...
            /* Read data */
            err = g_zmod4xxx_sensor2.p_api->read(g_zmod4xxx_sensor2.p_ctrl, &oaq_raw_data);
            if (FSP_SUCCESS == err)
            {
        ...
        {
            /* Calculate data */
            err = g_zmod4xxx_sensor2.p_api->oaq1stGenDataCalculate
                    (g_zmod4xxx_sensor2.p_ctrl, &oaq_raw_data, &zmod4xxx_data);
            if (FSP_SUCCESS == err)
            {
                sequence = DEMO_SEQUENCE_1;
                /* Set data */
...
/**********************************************************************************************************************
 End of function start_oaq_1st_gen_demo
 *********************************************************************************************************************/

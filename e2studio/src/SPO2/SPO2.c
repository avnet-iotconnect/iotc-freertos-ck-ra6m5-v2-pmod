...
void do_algorithm_part2(struct oxim *_ox, struct ob1203 *_ob, struct spo2 *_sp)
{
    ...
    { //original algorithm part 2 (RMS-based)
        /*increment the algorithm run count. Like a 1 second tick. Keeps track of peak and valley positions. */
        _sp->alg_count++;
    ...
    #endif
#endif
        /*get rough and reliable estimate of heart rate using zero crossings relative to a threshold
         * at 1/8 of max signal in the center half of the IR data buffer */
        ...
        {//pick the higher of the two threshold crossing counts (missing beats due to breathing is higher risk)
            _sp->hr_zc = (hr_zc_p > hr_zc_n) ? hr_zc_p : hr_zc_n;
        }
        else
        {//pick the lower of the two threshold crossing counts (overcounting due to dicrotic notch is higher risk)
            _sp->hr_zc = (hr_zc_n < hr_zc_p) ? hr_zc_n : hr_zc_p;
        }
        if (_sp->hr_zc > 0)
        {
        ...
        if (MIN_ALG_RUNS_SWITCH_TO_LONGER_AVG == _sp->alg_count)
        {
...
/*extends useful range of SpO2 at low PI -- MAX_NOISE must be characterized per hardware and supply source. */
#ifdef USE_NOISE_CORRECTION
        /*Can cause jumping in ppg_rms1f values when signal gets close to noise because
         * if noise is bigger then the subtraction won't occur*/
...
/**********************************************************************************************************************
 * Function Name: do_algorithm_part3
 * Description  : This function does breathing rate detection by detecting periodic variations in heart rate signals.
...
/**********************************************************************************************************************
 End of function do_algorithm_part3
 *********************************************************************************************************************/

/**********************************************************************************************************************
 * Function Name: do_algorithm_part4
 * Description  : Part 4 determines the most likely accurate baseline for RR AM zero crossings, to determine RR.
 * It uses an efficient IIR approximation of RMS and a merit function (badness) to track
 * which baseline is most likely to be accurate. A damping filter is used to prevent rapid changes in baseline.
 * Argument     : _sp
 * Return Value : .
 *********************************************************************************************************************/
...
/******************************************************************************
 * Function Name: sign
 * Description  : This function returns the sign of a number.
 * Argument     : val
 * Return Value : sign (1 = positive, 0 = zero, -1 = negative
 *****************************************************************************/
...
/**********************************************************************************************************************
 * Function Name: get_avg_PI
 * Description  : This function performs an average of recent PI values to reduce display variation
 * Arguments    : new_PI
 *              : pi_rms
 * Return Value : average PI
 *********************************************************************************************************************/
...
/**********************************************************************************************************************
 * Function Name: compare_hr
 * Description  : Decides whether to use beat-to-beat HR period or threshold crossing HR period
 * Arguments    : _sp
...
/**********************************************************************************************************************
 End of function compare_hr
 *********************************************************************************************************************/


/**********************************************************************************************************************
 * Function Name: high_pass_filter
 * Description  :  Does a centered baseline removal from AC1f. Last baseline_length-1 points are
 *                  not baseline-removed (unmodified)
 * Arguments    : _sp
 *              : baseline_bits
 *              number of bits to use for the baseline (baseline_length = 1<<baseline_bits)
 *              : new_array_len
 *              pointer to variable to update with the useful array length of the filtered array
 * Return Value : average PI
 *********************************************************************************************************************/
int8_t high_pass_filter(struct spo2 *_sp, uint16_t baseline_bits, uint16_t *new_array_len)
{
     ...
     * for example an array of 16 pts with a baseline of 3 will return 14 pts, each an average of the previous,
...
void get_rms_classic(struct spo2 *_sp, uint16_t array_len, uint16_t rms_len)
{
    if (rms_len == 0 || rms_len > array_len || array_len == 0 || rms_len == 0)
        ...
        for (uint8_t channel = 0; channel < _sp->ppg2+1; channel++)
        {
            ...
            {//functions same as RMS but easier to compute
                var1f += (uint32_t) (labs( (int32_t) AC1f[channel][n]) * labs((int32_t)_sp->AC1f[channel][n]));
            }
...
{
    if ((0 == rms_len) || (rms_len > array_len) || (0 == array_len) || (0 == rms_len))
    {
        return;
    }
...
void get_pressure(struct spo2 *_sp)
{
    ...
    {//64x ratio of systole to diastole (higher ratio = higher pressure = more + correction to SpO2 needed
        _sp->pressure_factor = ((labs(_sp->systole_trend) << FIXED_BITS)<<FIXED_BITS) / _sp->diastole_trend;
    }
}
...
void do_noise_correction(struct spo2 *_sp, struct ob1203*_ob)
{
    ...
    #endif
        for (uint8_t m=0; m < (_sp -> ppg2 + 1); m++)
...
/**********************************************************************************************************************
 End of function do_noise_correction
 *********************************************************************************************************************/
#endif
...
/**********************************************************************************************************************
 End of function round_dec_spo2
 *********************************************************************************************************************/


/**********************************************************************************************************************
 * Function Name: do_algorithm
 * Description  : .
 * Arguments    : _ox
 *              : _ob
 *              : _sp
...
#ifdef CHECK_PI
    check_pi(_ox,_sp);  /*adds a comment to the UART print,*/
                        /*freezes Spo2 if new data is questionable, resets alg if it gets too bad*/
#endif
}
/**********************************************************************************************************************
 End of function do_algorithm
 *********************************************************************************************************************/
...
void check_pi(struct oxim *_ox, struct spo2 *_sp)
{
    ...
    if ((_sp->ppg_rms1f[IR] != 0) && (_sp->perfusion_index[IR] > 0))
    {
        ...
        if (((_sp->ppg_rms1f[IR] >> (uint8_t)FIXED_BITS) < (uint32_t)MIN_RMS)
            || (_ox->latest_ir_data < (uint32_t)8000) || (_sp->perfusion_index[IR] < (float)MIN_PI))
        {
        ...
        /* check if PI is so low we need a stricter standard for RMS variation*/
        /*(need less motion than we can typically tolerate)*/
        else if (_sp->perfusion_index[IR] < (float)LOW_PI_LEVEL)
        {
            ...
            else if ((_sp->alg_count >= MIN_ALG_RUNS) &&
                    ((_sp->ppg_rms1f[IR] >> (uint8_t)FIXED_BITS) > (uint32_t)MIN_RMS))
            {
                _sp->no_disp_spo2_cnt = 0;
                _sp->spo2_err = spo2_err_none; //normal, okay
            }

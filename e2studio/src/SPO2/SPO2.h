...
/****************Sample rate-dependent parameters defining HR data buffer size and startup****************************/
    ...
    #define ARRAY_LENGTH        (320)   /*set this to at least 2*ARRAY_LENGTH. */
                                        /*Threshold crossing count duration will be this minus - 63. */
    ...
    #define STABILITY_PERIOD    (200)  /*add an extra number of samples to collect before analyzing
                                        (finger settling motion, signal drift, etc reduce initial accuracy)*/
    #define SAMPLE_RATE         (100)   /*100 samples per second --also adjust SAMPLE_RATE_PER_MINUTE if you change this */
    #define SAMPLE_RATE_PER_MIN 6000    /*samples per minute (set to 60 * sample rate) */
#else
    #define SAMPLE_LENGTH       (74)      //40.5 BPM at 50sps,
    #define MAX_WINDOW_LENGTH   32
    #define MIN_WINDOW_LENGTH   6
    #define ARRAY_LENGTH        (164)   //must be at least twice sample length
                                        //--can reduce this a little to save some RAM and compute time.
                                        //Longer is better for lower heart rates, but compute time increases.
    #define MIN_HR_PERIOD       (15)    //max HR is 3000/MIN_HR_PERIOD, so 16 = 187.5 bpm
    #define MAX_HR_PERIOD       (SAMPLE_LENGTH) //min HR is 6000/MAX_HR_PERIOD, so 148 = 41 bpm
    #define STABILITY_PERIOD    (100)   //add an extra number of samples to collect before analyzing
                                        //(finger settling motion, signal drift, etc reduce initial accuracy)
    #define SAMPLE_RATE         (50)    //samples per second --also adjust SAMPLE_RATE_PER_MINUTE if you change this
    #define SAMPLE_RATE_PER_MIN (3000)  //samples per minute
...
#define MIN_ALG_RUNS                        (7) /*minimum number of times*/
                                                /*the algorithm has to run before the display update.  */
                                                /*Increases startup time, but increases accuracy of initial value.*/
...
typedef struct spo2 {
    ...
    /*including 4 bits fixed point after subtracting local mean (max AC amplitude
     * (half of P2P signal must be less than 2^(15-4) = 2^11 = 2048) */
    int16_t AC1f[2][ARRAY_LENGTH];

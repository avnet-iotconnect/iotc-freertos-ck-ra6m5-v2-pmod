...
/*this sends a single character after each read data to acknowledge receipt
 * (necessary as some PC com port drivers have inconsistent UART timing) */
#define HANDSHAKE
...
/************STEP 2: Compute HR only, Compute SP02 and HR only, OR include respiration rate (RR)  */
/**************************(~3-5x increase in computation)*****************************************/
/* For respiration rate, in the file SPO2.h include the line #define COMPUTE_RR.
 * Comment out #define COMPUTE_RR to run algorithm part 1 every INTERVAL 100 (1 sec)
 * instead of INTERVAL 40 for ~30% computation time savings.
...
/*************STEP 8: Auto power off. For systems which need to turn themselves off after a timeout
 ******************** (not enabled on RL2_EVK EVK)*************************************************/
#ifndef RL2_EVK
...
typedef struct oxim {
    ...
    uint8_t                 mode; //start in prox mode, then switch to HR mode when detecting the proximity

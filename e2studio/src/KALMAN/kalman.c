...
/**********************************************************************************************************************
 End of function get_avg
 *********************************************************************************************************************/


/**********************************************************************************************************************
 * Function Name: run_kalman
 * Description  : .
 * Arguments    : num
 *              : new_data
 *              : ratio
...
void run_kalman(uint8_t num, uint32_t new_data, uint8_t ratio)
{
    ...
    {
    //not resetting and valid sample case (typical case)
    ...
    if (ratio)
    {
        if(KF.data_std < ((int32_t)KF.min_data_std * (int32_t)KF.kalman_avg) / 128)
        {
            //min_data_std is express in terms of 1/128 fraction of the kalman_avg
            KF.data_std = ((int32_t)KF.min_data_std * (int32_t)KF.kalman_avg) / 128;
        }
    } else
    {//min_data_std is expressed as an absolute number
    ...
    outlier = 0;
    if (new_data > KF.kalman_avg)
    {
        /*new data is bigger than kalman average */
        up_thresh = KF.kalman_threshold_2x*KF.data_std;
        if (KF.jumps_ok == 0) up_thresh = up_thresh >> 1; //divide by two for normal case
        if (new_data - KF.kalman_avg > up_thresh)
        {
        /*jumping high case for SpO2 */
          KF.outlier_cnt++;
          outlier = 1;
        }
    }else
    {
        /*new_data is smaller than kalman_avg */
    ...
    if (outlier == 0)
    {
        /*not an outlier, update the filter */
...
/**********************************************************************************************************************
 End of function run_kalman
 *********************************************************************************************************************/


/**********************************************************************************************************************
 * Function Name: uint8_t num
 * Description  : This is function to throw small data points (noise) out of the average
...
void get_big_avg(uint8_t num)
{
    ...
    for (uint8_t n = 1; n < KF.data_array_length; n++)
    {
        /*get the maximum */
    ...
    for (uint8_t n = 0; n < KF.data_array_length; n++)
    {
        /*include greater than half the max */
        if (n != max_ind)
        {
